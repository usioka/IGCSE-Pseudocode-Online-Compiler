...
describe('observeRun', () => {
  it('records one step per executed statement, with output attached to the right step', async () => {
    const obs = await observeRun('DECLARE Total : INTEGER\nTotal <- 0\nOUTPUT Total\n');
  ...
  it('tracks a variable changing value across steps', async () => {
    const obs = await observeRun('DECLARE Total : INTEGER\nTotal <- 0\nTotal <- Total + 5\nOUTPUT Total\n');
    const totalValues = obs.steps.map((s) => s.variables.find((v) => v.name === 'Total')?.value).filter(Boolean);
  ...
  it('feeds queued inputs to INPUT requests in order', async () => {
    const obs = await observeRun('DECLARE Name : STRING\nINPUT Name\nOUTPUT Name\n', { inputs: ['Ada'] });
    expect(obs.completed).toBe(true);
