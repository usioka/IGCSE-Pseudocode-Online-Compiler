...
async function check(requirement: string, pseudocode: string, inputs?: string[]) {
...
describe('RequirementWatchdog — Type 1 (autonomous system activity)', () => {
  ...
  it('violates when the object never appears in the program at all', async () => {
    const verdict = await check('The System must calculate Total.', 'DECLARE X : INTEGER\nX <- 1\nOUTPUT X\n');
    expect(verdict.status).toBe('violated');
  });
  ...
  it('violates when the run itself errored', async () => {
    const verdict = await check('The System must calculate X.', 'DECLARE X : INTEGER\nX <- 1 / 0\n');
    expect(verdict.status).toBe('violated');
  });
  ...
  it('checks each requirement in a multi-requirement document, worst status wins', async () => {
    ...
    expect(errors).toEqual([]);
    const observation = await observeRun('DECLARE Total : INTEGER\nTotal <- 0\nOUTPUT Total\n');
...
describe('RequirementWatchdog — Type 2 (user interaction)', () => {
  const REQUIREMENT = 'The System must offer the user the possibility to open Door.';
